...
export default function RoomsManagement() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // ALWAYS generate a unique room number when creating a new room
      // This helps prevent duplicate key errors
      if (!isEditing) {
        const timestamp = Date.now();
        const randomDigits = Math.floor(Math.random() * 1000).toString().padStart(3, '0');
        
        // Use user's room number as prefix if provided, or 'R' if empty
        const prefix = currentRoom.roomNumber && currentRoom.roomNumber.trim() !== '' ? 
                      currentRoom.roomNumber.trim() : 'R';
                      
        // Create unique number by appending timestamp and random digits
        const uniqueRoomNumber = `${prefix}-${timestamp.toString().slice(-4)}-${randomDigits}`;
        formData.append('roomNumber', uniqueRoomNumber);
        console.log(`Generated unique room number: ${uniqueRoomNumber}`);
      } else {
        // For editing, keep the existing room number
      ...
      // Handle array fields by safely converting them to JSON strings
      try {
        // Process amenities
        ...
        console.error("Error stringifying arrays:", jsonError);
        // Fallback to empty arrays
        formData.append('amenities', JSON.stringify([]));
        formData.append('additionalAmenities', JSON.stringify([]));
      ...
      // Determine the API URL and method based on whether we're creating or editing
      ...
      console.log('Using direct backend connection URL:', url);

      // Debugging: log all form fields before cleaning
      console.log("Form fields before cleanup:", [...formData.keys()]);
      
      // IMPORTANT: Check if there's a discount field and remove it
      if (formData.has('discount')) {
        console.log("⚠️ Found and removing problematic 'discount' field");
        const cleanedFormData = new FormData();
        
        // Copy all fields except 'discount' to a new FormData
      ...
      // Try to get the auth token from localStorage first (most likely source)
      let userData = null;
      try {
      ...
      // Get the token from all possible storage locations with localStorage taking precedence
      // (based on your checkAdminAuth function's implementation)
      ...
      if (error instanceof Error) {
        ...
        if (apiError.message && apiError.message.includes("duplicate key") && 
            apiError.message.includes("rooms_room_number_key")) {
          setError(
            "This room number already exists. Please choose a different room number."
          );
        } else {
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="whitespace-nowrap flex space-x-2">
                          ...
                          <button
                            ...
                            onClick={() => router.push(`/admin/rooms/${room.id}/manage-images`)}
                            title="Manage Images"
                          >
